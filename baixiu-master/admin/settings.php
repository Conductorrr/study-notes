  ...
  <script>
    $(function () {
      ...
      $('#upload').on('change', function () {
        // 选择文件后异步上传文件
        // FormData是html5中新增的一个成员，专门用于配合ajax操作，用于在客户端和服务器之间传递二进制数据
        var formData = new FormData()
        formData.append('file', $(this).prop('files')[0]) //在数据末尾追加数据key为file，value为...的数据
        //第一个参数相当于表单的name属性值,第二个参数就是我们要上传的文件。服务器通过$_FILE['file]取到这个文件
